...
const HeroSection = () => (
    // Added pb-24 to push the content up from the bottom nav bar
    <section id="about" className="min-h-screen px-6 sm:px-12 md:px-24 flex flex-col-reverse md:flex-row items-center justify-center gap-12 relative overflow-hidden pb-24">
        {/* ... (background blur elements remain the same) ... */}
         <motion.div 
        ...
        />
        {/* Updated text alignment to md:text-left */}
        <motion.div className="flex-1 text-center md:text-left relative z-20" initial={{ opacity: 0, x: -50 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.8 }}>
            ...
            <h1 className="text-5xl md:text-7xl font-extrabold my-2">
                <span className="text-transparent bg-clip-text bg-gradient-to-r from-indigo-500 to-pink-500">
                    {portfolioOwner.name} 
                    <span className="text-lg md:text-xl align-middle">({portfolioOwner.lastName})</span>
                </span>
            </h1>
            ...
            {/* Updated justification to md:justify-start */}
        ...
        {/* Added margin-top (mt-12) to create space on mobile */}
        <motion.div className="flex-1 flex justify-center mt-12 md:mt-0" initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.8, delay: 0.2 }}>
        ...
        <a href="#skills" className="hidden md:block absolute bottom-10 left-1/2 -translate-x-1/2 text-gray-500 dark:text-gray-400 animate-bounce">
